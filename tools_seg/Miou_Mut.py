...
def mean_iou(pred, target, num_classes=3):
    ...
    return mean_iou
def recall_multiclass(input, target, num_classes):
    input = input.data.cpu().numpy()
    target = target.data.cpu().numpy()

    recalls = []
    for c in range(num_classes):
        TP = ((input == c) & (target == c)).sum()
        FN = ((input != c) & (target == c)).sum()
...
def precision_multiclass(input, target, num_classes):
    input = input.data.cpu().numpy()
    target = target.data.cpu().numpy()
...
def F1score_multiclass(input, target, num_classes):
    input = input.data.cpu().numpy()
    target = target.data.cpu().numpy()
...
def calculate_miou(input, target, classNum):
    ...
    for i in range(input.shape[0]):  # 遍历图像
        ...
        for j in range(classNum):  # 遍历类别，包括背景
            ...
            # 检查该类别在input和target中是否都不存在
            input_sum = torch.sum(inputOht[i][j])
            target_sum = torch.sum(targetOht[i][j])

            if input_sum == 0 and target_sum == 0:
                # 双方都不存在，认定为完美匹配，IoU=1
                iou = 1.0
            else:
                # 正常计算IoU
                iou = intersection / union
...
def precision(input, target):
    ...
    # 检查是否没有预测正样本（TP+FP=0）
    if TP + FP == 0:
        # 如果没有预测正样本，且真实标签中也没有正样本（FN=0），则precision为1
        if FN == 0:
            precision = 1.0
        else:
            precision = 0.0
