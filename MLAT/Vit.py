...
class VisionTransformer(nn.Module):  # Transformer-branch
    def __init__(self, config, vis, img_size, channel_num, patch_size, embed_dim, depth=1, num_heads=8,
        ...
        # self.text_conv = Text_Conv(in_channel=channel_num, text_dim=embed_dim)
        # self.cross_modal_fusion = CrossModalTransformerFusion(
        #     img_channels=channel_num,
        #     text_channels=embed_dim,
        #     img_size=img_size,
        #     patch_size=patch_size
        # )

        # 增强的序列融合（用于skip connection）
        # self.enhanced_skip = EnhancedSequenceFusion(
    ...
    def forward(self, x, skip_x, text, reconstruct=False):  # x = torch.Size([4, 64, 224, 224]), skip_x = torch.Size([4, 64, 224, 224]), text = torch.Size([4, 10, 64])二层([4, 128, 112, 112])([4, 196, 64])([4, 10, 128])
        ...
        if not reconstruct:  # 64.先来这
            # print("not")
            # x = self.cross_modal_fusion(x, text)
            # x = self.text_conv(x, text)
        ...
        if (self.dim == 64 and not reconstruct) or (self.dim == 512 and reconstruct):
            ...
            # print(x.size())
            y = torch.cat([x, skip_x+abs(x - skip_x)], dim=2)
            # print(y.size())
            return y
        elif reconstruct:
            ...
            # x1 = self.x16_3(skip_x)
            # x1 = x1.transpose(1, 2)
            # print(skip_x.size())
            skip_x = skip_x.transpose(1, 2)
            # print(skip_x.size())
            y = x+skip_x+abs(x-skip_x)
            # print(y.size())
            return y